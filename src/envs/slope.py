...
class SlopeEnvParent:
    def __init__(self, num_robots=2, num_resources=3, sensor_range=1, slope_angle=40, arena_length=8, arena_width=4,
        ...
        self.tiles_in_sensing_range = (
                                                  2 * self.sensor_range + 1) ** 2  # Range=1 -> 9 tiles. Range=2 -> 25 tiles. Robot at the center.
    ...
    def step(self, robot_actions, time_step):
        ...
        for i in range(self.num_robots):
            if self.get_area_from_position(self.robot_positions[i]) == "NEST" and self.has_resource[i] is None and \
                    self.robot_positions[i] in self.resource_positions:
                # self.has_resource[i] = None
                resource_id = self.resource_positions.index(self.robot_positions[
                                                                i])  # Find the resource with the same position as the current robot and get that resource's id
