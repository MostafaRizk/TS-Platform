...
def generate_constants(parameter_dictionary):
    ...
    cost["Generalist"] = parameter_dictionary["generalist_base_cost"] + parameter_dictionary[
        "generalist_cost_multiplier"] * slope
    cost["Dropper"] = parameter_dictionary["dropper_base_cost"] + parameter_dictionary[
        "dropper_cost_multiplier"] * slope
    cost["Collector"] = parameter_dictionary["collector_base_cost"] + parameter_dictionary[
        "collector_cost_multiplier"] * theta(slope)
...
def get_payoff(team, all=False):
    ...
    if num_collectors != 0:
        ...
        if pair_alignment_probability == 1.0:
            ...
            available_spaces = [original_available_spaces] + [j for j in range(original_available_spaces,
                                                                               original_available_spaces - num_spec_pairs - 1,
                                                                               -1)]

            total_probability = 0

            for i in range(1, num_spec_pairs + 1):
                total_probability += pair_alignment_probability * (base_arena_width / available_spaces[0]) * (
                            available_spaces[i - 1] / available_spaces[i]) * (num_spec_pairs - i + 1)
...
def get_many_final_distributions(parameter_dictionary):
    ...
    if 0.0 <= proportion_of_novices <= 1.0:
        subset_distributions = (1.0 - proportion_of_novices) * sample_distributions(num_samples,
                                                                                    subset_of_population=True)
...
def plot_trend(parameter_dictionary, P0, path):
    generate_constants(parameter_dictionary, team_size=parameter_dictionary["default_num_agents"],
                       slope=parameter_dictionary["default_slope"])
...
def plot_distribution_frequency(parameter_dictionary, path, distribution_dictionary, plot_type=None):
    ...
    if not plot_type:
        ...
        plt.plot(team_sizes, cooperation_rates, marker='o', markersize=markersize, linewidth=linewidth,
                 color=equilibrium_colour["Cooperative Equilibrium"])
